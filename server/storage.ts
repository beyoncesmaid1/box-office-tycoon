...
export interface IStorage {
  ...
  getStudiosByDeviceId(deviceId: string): Promise<Studio[]>;
  getPlayerStudioByDeviceId(deviceId: string): Promise<Studio | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getAllFilmReleases(): Promise<FilmRelease[]> {
    return await db.select().from(filmReleases);
  }
  ...
  async getPremiumBooking(id: string): Promise<PremiumBooking | undefined> {
    const [booking] = await db.select().from(premiumBookings).where(eq(premiumBookings.id, id));
    return booking;
  }
  ...
  async getAllPremiumBookings(): Promise<PremiumBooking[]> {
    return await db.select().from(premiumBookings);
  }
  ...
  async updatePremiumBooking(
    ...
    const [updated] = await db.update(premiumBookings)
      .set(updates)
      .where(eq(premiumBookings.id, id))
      .returning();
    return updated;
  }

  async deletePremiumBooking(id: string): Promise<void> {
    await db.delete(premiumBookings).where(eq(premiumBookings.id, id));
  }

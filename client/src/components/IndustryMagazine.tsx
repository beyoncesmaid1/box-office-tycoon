...
import { useGame, formatMoney, genreLabels, asArray } from '@/lib/gameState';
...
export function IndustryMagazine() {
  ...
  const { data: allFilmsData } = useQuery<FilmType[]>({
    queryKey: ['/api/all-films', state.studioId],
  });
  
  const { data: allStudiosData } = useQuery<Studio[]>({
    queryKey: ['/api/studios', state.studioId],
  });
  
  const { data: allTalentData } = useQuery<Talent[]>({
    queryKey: ['/api/talent'],
  });
  
  const { data: tvShowsData } = useQuery<TVShow[]>({
    queryKey: ['/api/tv-shows', { studioId: state.studioId }],
    queryFn: async () => {
      const response = await fetch(`/api/tv-shows?studioId=${state.studioId}`);
      const data = await response.json();
      return asArray<TVShow>(data);
    },
    enabled: !!state.studioId,
  });
  
  const { data: awardShowsData } = useQuery<AwardShow[]>({
    queryKey: ['/api/award-shows'],
  });
  
  const { data: nominationsData } = useQuery<AwardNomination[]>({
    queryKey: ['/api/nominations', state.studioId],
    enabled: !!state.studioId,
  });
  
  const { data: ceremoniesData } = useQuery<AwardCeremony[]>({
    queryKey: ['/api/ceremonies', state.studioId],
    enabled: !!state.studioId,
  });

  const allFilms = asArray<FilmType>(allFilmsData);
  const allStudios = asArray<Studio>(allStudiosData);
  const allTalent = asArray<Talent>(allTalentData);
  const tvShows = asArray<TVShow>(tvShowsData);
  const awardShows = asArray<AwardShow>(awardShowsData);
  const nominations = asArray<AwardNomination>(nominationsData);
  const ceremonies = asArray<AwardCeremony>(ceremoniesData);
  ...
  const allStories = useMemo(() => {
    ...
    for (let weeksAgo = 0; weeksAgo < 12; weeksAgo++) {
      ...
      const storyWeekIndex = storyYear * 52 + storyWeek;
      const recordBreakingFilms = allFilms.filter(film => {
        if (!film.releaseWeek || !film.releaseYear || film.totalBoxOffice <= 0) return false;
        const filmReleaseIndex = film.releaseYear * 52 + film.releaseWeek;
        if (filmReleaseIndex !== storyWeekIndex) return false;
        const priorFilms = allFilms.filter(candidate =>
          candidate.totalBoxOffice > 0 &&
          candidate.releaseWeek &&
          candidate.releaseYear &&
          candidate.releaseYear * 52 + candidate.releaseWeek < filmReleaseIndex
        );
        if (priorFilms.length === 0) return false;
        const previousRecord = Math.max(...priorFilms.map(candidate => candidate.totalBoxOffice || 0));
        return film.totalBoxOffice > previousRecord;
      });
      const recordFilm = recordBreakingFilms
        .sort((left, right) => (right.totalBoxOffice || 0) - (left.totalBoxOffice || 0))[0];
      if (recordFilm && weeksAgo <= 4) {
        const studio = studioMap.get(recordFilm.studioId);
        stories.push({
          id: `record-w${storyWeek}-y${storyYear}-${recordFilm.id}`,
          type: 'record-break',
          headline: `"${recordFilm.title}" Sets a New Worldwide Box Office Record`,
          content: `${studio?.name || 'A major studio'}'s "${recordFilm.title}" has reached ${formatCompactMoney(recordFilm.totalBoxOffice)}, surpassing every earlier release in this industry's recorded history.`,

...
import { useGame, formatMoney, genreColors, genreLabels, getWeeklyBoxOffice, asArray, type FilmWithTalent } from '@/lib/gameState';
...
export function BoxOfficeLeaderboard() {
  const { state } = useGame();
  const { data: allFilmsData } = useQuery<Film[]>({
  ...
  const allFilms = asArray<Film>(allFilmsData);
  ...
  const sortedFilms = [...allFilms]
    .filter(f => getWeeklyBoxOffice(f).length > 0)

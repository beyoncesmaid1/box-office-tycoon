...
export const films = pgTable("films", {
  ...
  campaignLimit: bigint("campaign_limit", { mode: "number" }).notNull().default(0),
  campaignSpent: bigint("campaign_spent", { mode: "number" }).notNull().default(0),
  campaignStrategy: text("campaign_strategy").notNull().default("balanced"),
  autoManageMarketing: boolean("auto_manage_marketing").notNull().default(false),
  ...
  imaxSuitability: integer("imax_suitability").notNull().default(0),
  dolbySuitability: integer("dolby_suitability").notNull().default(0),
  boxOfficeModelVersion: integer("box_office_model_version").notNull().default(2),
  awards: text("awards").array().notNull().default(sql`ARRAY[]::text[]`),
  franchiseId: varchar("franchise_id").references((): AnyPgColumn => franchises.id),
  isSequel: boolean("is_sequel").notNull().default(false),
  prequelFilmId: varchar("prequel_film_id").references((): AnyPgColumn => films.id),
...
export const franchises = pgTable("franchises", {
  ...
  studioId: varchar("studio_id").notNull().references(() => studios.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
  originalFilmId: varchar("original_film_id").notNull().references((): AnyPgColumn => films.id),
...
export const filmReleases = pgTable("film_releases", {
  ...
  productionBudget: bigint("production_budget", { mode: "number" }).notNull().default(0),
  marketingBudget: bigint("marketing_budget", { mode: "number" }).notNull().default(0),
  awareness: real("awareness").notNull().default(8),
  interest: real("interest").notNull().default(10),
  expectation: real("expectation").notNull().default(52),
  buzz: real("buzz").notNull().default(0),
  paidReach: real("paid_reach").notNull().default(0),
  openingExpectation: real("opening_expectation"),
  isReleased: boolean("is_released").notNull().default(false),
  weeklyBoxOffice: bigint("weekly_box_office", { mode: "number" }).array().notNull().default(sql`ARRAY[]::bigint[]`),
  weeklyCapacityBreakdown: jsonb("weekly_capacity_breakdown").notNull().default(sql`'[]'::jsonb`),
  totalBoxOffice: bigint("total_box_office", { mode: "number" }).notNull().default(0),
  theaterCount: integer("theater_count").notNull().default(0),
...
export const marketingActions = pgTable("marketing_actions", {
  ...
  territoryCode: text("territory_code").notNull(),
  actionKind: text("action_kind").notNull(),
  spend: bigint("spend", { mode: "number" }).notNull(),
  response: real("response").notNull().default(1),
  reachGain: real("reach_gain").notNull().default(0),
  week: integer("week").notNull(),
  year: integer("year").notNull(),
  stateAfter: jsonb("state_after").notNull().default(sql`'{}'::jsonb`),
});

export const insertMarketingActionSchema = createInsertSchema(marketingActions).omit({ id: true });
...
export const premiumBookings = pgTable("premium_bookings", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  filmId: varchar("film_id").notNull().references(() => films.id, { onDelete: "cascade" }),
  format: text("format").notNull(), // imax | dolby
  territoryCode: text("territory_code").notNull(),
  accessLevel: text("access_level").notNull(), // standard | priority | exclusive
  startWeek: integer("start_week").notNull(),
  startYear: integer("start_year").notNull(),
...
export const insertPremiumBookingSchema = createInsertSchema(premiumBookings).omit({ id: true });
export type InsertPremiumBooking = z.infer<typeof insertPremiumBookingSchema>;
export type PremiumBooking = typeof premiumBookings.$inferSelect;
